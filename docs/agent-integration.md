...
7. 需要为已有 `secret://` 增加精确服务目标和协议时，使用 `secret_bind_destination`；它会在 App 中显示本次绑定并要求每次重新进行 device-owner authentication，不返回明文，也不建立执行窗口。
...
| 为已有 Secret 绑定服务目标 | `secret_bind_destination` | 传入已有 `secret://`、精确目标和协议；每次需要 device-owner authentication，重新封装绑定元数据，不返回明文。 |
...
| `secret_bind_destination` | Adds one exact service destination/protocol to an existing `secret://` record after fresh device-owner approval; re-seals encrypted metadata and never returns plaintext. |
