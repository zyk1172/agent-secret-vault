...
7. 没有安全工具时停止并请求新增 allowlisted 工具，不降级为通用命令、浏览器填表或索要明文。
...
| 模板化本地执行 | `secure_execute` | 仅限 allowlist 模板；可能返回 `EXECUTE_UNAVAILABLE`。 |
...
| `vault_status` | Checks whether the app is reachable and whether the vault is locked. |
...
| `secure_execute` | Compatibility endpoint for future allowlisted local execution; may return `EXECUTE_UNAVAILABLE`. |
...
- 返回 `EXECUTE_UNAVAILABLE`、`SELECTION_ENCRYPT_UNAVAILABLE`：说明当前桥接能力不可用；不要改用明文替代。
