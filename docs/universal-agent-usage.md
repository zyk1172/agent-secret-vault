...
13. `敏感信息.md` 分为前言区、连续的 Catalog 主体区和尾部非托管 Markdown。Note、说明、callout、普通段落、备注和 `[[WikiLink]]` 都是 unmanaged，不属于 Catalog semantic model。存在 Index 时，新 Index 插入最后一个合法 `SVLT-INDEX` 后、尾部非托管 Markdown 前；没有 Index 时插入前言后，不得简单 append 到文件末尾。
14. Index 之间使用 renderer 生成的 `\n\n---\n\n`，`---` 仅是视觉边界；同一 Index 内 Entry 使用统一双空行视觉间距。所有合法 writer（App、MCP、Obsidian、编辑器、脚本）都必须遵守 v3 marker/schema，受控写入优先 minimal patch 并保留用户 Markdown。format repair 不能把无法确认的用户 Note 自动搬走。Agent 不得读取 selection JSON、Catalog Markdown 或 Application Support sidecar 猜测 opaque ID，也不得读取本地 `sensitive-index-selection.json`、integrity sidecar 或其他 Application Support 文件解析 ID。
