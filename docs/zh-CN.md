...
如果 `敏感信息.md` 损坏或完整性校验失败，在 App 中点击“修复 / 恢复”。App 会先显示恢复预览，包含当前状态、目标快照版本、分组/条目/密码引用数量以及语义变化；确认后 SVLT 要求本机身份认证，并在提交前再次校验当前文件哈希。预览之后文件发生变化会拒绝恢复，不会覆盖。
...
重点：Obsidian 插件是只读格式校验器，SVLT App 才是密码填写、查看、编辑、恢复和高风险审批的中心。
