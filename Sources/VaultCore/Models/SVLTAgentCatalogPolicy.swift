...
public enum SVLTAgentCatalogPolicy {
    ...
    public static let legacyDocumentPolicyBlock: String = {
        let currentRule = "> 38. 格式修复只能调整格式，不能改变结构或 opaque 引用，不能生成或展开明文。"
        let legacyRule = "> 38. 恢复功能只能恢复结构和 opaque 引用，不能生成或展开明文。"
        return documentPolicyBlock
    ...
    private static func makeLegacyDocumentPolicyBlock(
        ...
        let transformed = documentPolicyBlock
