...
public enum SVLTAgentCatalogPolicy {
    /// Exact policy emitted by the release immediately before automatic
    /// device-owner authentication. The complete text is pinned so format
    /// repair can recognize it without accepting an arbitrary digest.
    private static let preAutoAuthorizationPolicyText = """
    SVLT 敏感信息目录写入规范
    ...
    public static let preAutoAuthorizationDocumentPolicyBlock = makePolicyBlock(
        text: preAutoAuthorizationPolicyText,
        beginMarker: "<!-- SVLT-POLICY-BEGIN version=\"3\" digest=\"\(preAutoAuthorizationDocumentPolicyDigest)\" -->"
    )

    /// The canonical policy block emitted by the PR #19 base release. Keep
    /// the complete block fixed so an upgrade can recognize this exact prior
    /// version without accepting an arbitrary policy digest or text shape.
    public static let previousDocumentPolicyDigest = "5259128b1df47f9e24b46892186654f02c9793fb0e0318b016b4b8a135b3e595"
    public static let previousDocumentPolicyBeginMarker = "<!-- SVLT-POLICY-BEGIN version=\"3\" digest=\"\(previousDocumentPolicyDigest)\" -->"
    public static let previousDocumentPolicyBlock = """
    ...
    /// Earlier v3 policy shapes were emitted before the current
