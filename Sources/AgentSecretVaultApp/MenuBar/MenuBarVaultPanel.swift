                ...
                ForEach(entries) { entry in
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text(entry.target)
                            ...
                            .foregroundStyle(.secondary)
                            .lineLimit(2)
                        Text("\(entry.source.displayName) · \(entry.authorizationMode?.displayName ?? entry.authorizationOutcome.displayName) · \(entry.timestamp.formatted(date: .omitted, time: .standard))")
                            .font(.caption2)
