...
public enum VaultUICopy {
    ...
    public static let overviewSubtitle = BilingualText(
        ...
        chinese: "把知识库里的敏感文本加密成不透明的 secret:// 引用。智能体可以传递引用；明文查看、本机授权和受控发送都由本应用完成。"
    )
    ...
    public static let securityBoundaries: [SecurityBoundaryCopy] = [
        SecurityBoundaryCopy(id: "agent-plaintext", symbolName: "eye.slash", englishTitle: "Plaintext stays local by default", englishBody: "Temporary reveal does not return decrypted values to the plugin. Restore is explicit and writes plaintext back to Obsidian after local authorization.", chineseTitle: "默认明文留在本机", chineseBody: "临时解密不会把明文返回给插件。还原是显式操作，会在本机授权后把明文写回 Obsidian。", isLimitation: false),
