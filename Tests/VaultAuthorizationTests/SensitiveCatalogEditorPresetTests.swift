...
@Test func catalogEditorUsesDirectCardsAndEntryBatchEditing() throws {
    ...
    #expect(!source.contains("Button(isSelectingIndexes ? \"完成\" : \"选择\")"))
    #expect(!source.contains("Button(\"选择\")"))
    #expect(!source.contains("isSelectingIndexes"))
    #expect(!source.contains("selectedIndexIDs"))
    #expect(!source.contains("批量编辑分组"))
    #expect(!source.contains("删除选中分组"))
    #expect(source.contains("isSelectingEntries"))
    #expect(source.contains("selectedEntryIDs"))
    #expect(source.contains("批量编辑条目"))
    #expect(source.contains("prepareEntryDeletion()"))
    #expect(source.contains("完成批量编辑"))
    ...
    #expect(source.contains(".accessibilityHint(\"查看该分组中的条目\")"))
